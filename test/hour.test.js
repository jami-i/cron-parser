...
const Hour = require('../lib/Hour.js');

describe('hour parser', function() {
  
  describe("parsing", () => {
    it('can parse "*" pattern', function() {
      const type = typeof Hour.tryParse("*");
      assert.ok(type === 'function');
    });

    it('can parse static number pattern 0', function() {
      const type = typeof Hour.tryParse("0");
      assert.ok(type === 'function');
    });
  
    it('can parse static number pattern 23', function() {
      const type = typeof Hour.tryParse("23");
      assert.ok(type === 'function');
    });

    it('cannot parse static number pattern 24', function() {
      const type = typeof Hour.parse("24").value;
      assert.ok(type !== 'function');
    });
  
    it('can parse static or static pattern', function() {
      const type = typeof Hour.tryParse("1,12");
      assert.ok(type === 'function');
    });
  
    it('can parse range pattern', function() {
      const type = typeof Hour.tryParse("1-3");
      assert.ok(type === 'function');
    });

    it('can parse range or range pattern', function() {
      const type = typeof Hour.tryParse("1-3,9-12");
      assert.ok(type === 'function');
    });

    it('can parse static or range pattern', function() {
      const type = typeof Hour.tryParse("1,3-5");
      assert.ok(type === 'function');
    });

    it('can parse range or static pattern', function() {
      const type = typeof Hour.tryParse("1-5,10");
      assert.ok(type === 'function');
    });
  });
  ...
  describe('validator', function() {
    describe('static number', () => {
      it('static number match', function() {
        const validator = Hour.tryParse("1");
        assert.ok(validator(1) === true);
      });
  
      it('static number mismatch -1', function() {
        const validator = Hour.tryParse("2");
    ...
    describe("range operator", () => {
      it('range mismatch left -1', function() {
        const validator = Hour.tryParse("2-9");
        assert.ok(validator(1) === false);
      });
    ...
    describe("or operator" ,() => {
      it('or mismatch 1', function() {
        const validator = Hour.tryParse("1,11");
    ...
    describe("complex pattern", () => {
      it("ok", () => {
        const validator = Hour.tryParse("2-3,5,7,9-11");
